...
class AudioClassificationDataset(AudioDataset):
    ...
    def generate_records(self, input_size, ex_per_file=16, n_processes=multiprocessing.cpu_count()):
		...
		#self._write_params(n_steps=72041, length = 9.984, sample_rate=16000)
    ...
    def _record_generation_job(self, index, input_size, ex_per_file, offset, progress=False):
        ...
        for i, data in enumerate(index):
            ...
            try:
                ...
                print(file_x)
                std_out = sys.stdout
                with open('logvelocity.txt', 'a') as f:
                    sys.stdout = f
                    print(file_x)
                    sys.stdout = std_out
            ...
            #np.pad(csv_data, ((0,1),(0,0))) #potentially pad things differently
    ...
    def _load_map(self, proto_buff, output_size):
        ...
        return [x, y] #xx is audio autoencoder, yy is midi autoencoder xy is audio to midi
